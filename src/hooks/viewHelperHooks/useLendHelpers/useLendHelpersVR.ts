...
export const useLendHelpersVR = (
  ...
  const [fyTokenMarketValue, setFyTokenMarketValue] = useState<string>(); // TODO do we need this? update to vyToken maybe? - jacob b

  /* Roll state */
  const [maxRoll, setMaxRoll] = useState<BigNumber>(ethers.constants.Zero);
  const [maxRoll_, setMaxRoll_] = useState<string>();

  const { apr: apy } = useApr(input, ActionType.LEND, series); // TODO - might need to update when marco finishes his work - Jacob B
