...
export const useCollateralHelpersVariableRate = (
  ...
  useEffect(() => {
    ...
    console.log('useCollateralHelpers VR', _existingCollateral, existingCollateralAsWei, newCollateralAsWei);
    ...
    const newDebt =
      debtInput && Math.abs(parseFloat(debtInput)) > 0
        ...
        : ethers.constants.Zero; // TODO: why is this not decimal18

    // console.log('newDebt in useCollateralHelpers', newDebt);
    ...
    // console.log('useCollateralHelpers VR', _totalDebt, ethers.utils.formatUnits(_totalDebt, 18));
    // the above value seems right for totalDebt - Jacob b

    setTotalDebt(_totalDebt);
    setTotalDebt_(ethers.utils.formatUnits(_totalDebt, 18));

    console.log(
      'useCollateralHelpers VR before calc if/else',
      oraclePrice.gt(ethers.constants.Zero),
      _totalCollateral.gt(ethers.constants.Zero)
    );
    ...
    if (oraclePrice.gt(ethers.constants.Zero) && _totalCollateral.gt(ethers.constants.Zero)) {
      ...
      console.log(
        '%c in if useCollateralHelpers, trying to calc collat ratio',
        'font-size: 35px;',
        calculateCollateralizationRatio(_totalCollateral, oraclePrice, _totalDebt, false),
      ...
      console.log('%c in else useCollateralHelpers, setting collat ratio to 0', 'font-size: 48px;');
  ...
  console.log('%c useCollateralHelpers returns', 'color: #00ff00; font-size: 24px;', {
    collateralizationRatio,
