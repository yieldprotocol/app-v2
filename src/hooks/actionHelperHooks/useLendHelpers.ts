  ...
  useEffect(() => {
    ...
    if (series) {
      ...
      userBaseAvailable.lt(protocolBaseAvailable)
        ? setMaxClose(ethers.utils.formatUnits(userBaseAvailable, series.decimals).toString())
        : setMaxClose(ethers.utils.formatUnits(protocolBaseAvailable, series.decimals).toString());

      /* user balance or max Closable */
